...
func (s *TestSuite) TestOauthTokenByRefresh() {
	...
	refresh, err := s.app.Provider.Token().RefreshToken(
		context.Background(),
		session.Id,
	...
	refresh2, err := s.app.Provider.Token().RefreshToken(
		context.Background(),
		session.Id,
		s.app.Provider.Config().Client.Id,
		s.app.Provider.Config().Admin.Id,
	...
	testCases := []struct {
		...
		{
			name: "Successful token by refresh",
			query: map[string]string{
		...
		},
		{
			...
			expErr:  "token incorrect",
		},
		{
			name: "Query empty token",
			query: map[string]string{
				"grant_type":    "refresh_token",
				"refresh_token": "",
				"client_id":     s.app.Provider.Config().Client.Id,
		...
		{
			name: "Refresh token for another client",
			query: map[string]string{
				"grant_type":    "refresh_token",
				"refresh_token": refresh2.Hash,
